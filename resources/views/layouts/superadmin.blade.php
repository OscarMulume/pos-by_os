...
<head>
    ...
    <title>POS System — @yield('title', 'Super-Admin')</title>
...
<body class="h-full bg-animated-gradient" x-data="darkMode()" :class="{ 'dark': dark }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
           class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900/95 dark:bg-slate-950/95 backdrop-blur-xl text-white transform transition-transform duration-200 lg:translate-x-0 flex flex-col border-r border-slate-700/50">

        <!-- Logo -->
        <div class="flex items-center justify-between h-16 px-4 bg-slate-950/80">
            <a href="{{ route('superadmin.dashboard') }}" class="flex items-center space-x-2">
                <svg class="w-7 h-7 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                <span class="text-lg font-bold tracking-wide">POS <span class="text-amber-400">Pro</span></span>
            </a>
        ...
        <!-- Badge Super-Admin -->
        <div class="mx-3 mt-3 px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-lg">
            <p class="text-xs font-medium text-amber-400 text-center">⚡ Super-Admin</p>
        </div>

        <!-- Navigation -->
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            <a href="{{ route('superadmin.dashboard') }}"
               class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('superadmin.dashboard') ? 'bg-amber-600 text-white' : 'text-gray-300 hover:bg-slate-700 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                Dashboard Global
            </a>
            <a href="{{ route('superadmin.restaurants.index') }}"
               class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('superadmin.restaurants.*') ? 'bg-amber-600 text-white' : 'text-gray-300 hover:bg-slate-700 hover:text-white' }}">
            ...
            <div class="pt-4 pb-2 px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Système</div>
            <a href="{{ route('admin.dashboard') }}"
               class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition text-gray-300 hover:bg-slate-700 hover:text-white">
        ...
        <div class="p-4 border-t border-slate-700/50">
            <div class="flex items-center">
                ...
                <div class="ml-3 min-w-0">
                    ...
                    <p class="text-xs text-amber-400">Super-Admin</p>
                </div>
            </div>
        </div>
    </aside>
